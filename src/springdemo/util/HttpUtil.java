...
public class HttpUtil {
	...
	public HttpUtil(String sendUriStr) {
		this(URI.create(sendUriStr));
	}
	...
	public Map<String, String> sendPostRequest(){
		...
		try {
			...
			String host = sendUri.getHost();
			int port = sendUri.getPort() == -1 ? 80 : sendUri.getPort();
			 ...
			 * 构造HTTP请求报文
			 */
			reqMsg.append("POST ").append(sendUri.getPath()).append(" HTTP/1.1\r\n");
			reqMsg.append("Cache-Control: no-cache\r\n");
			reqMsg.append("Pragma: no-cache\r\n");
			reqMsg.append("User-Agent: JavaSocket/");
			reqMsg.append(System.getProperty("java.version")).append("\r\n");
			reqMsg.append("Host: ").append(host).append("\r\n");
			reqMsg.append("Accept: text/html, image/gif, image/jpeg, *; q=.2, */*; q=.2\r\n");
			reqMsg.append("Connection: keep-alive\r\n");
			reqMsg.append("Content-Type: application/x-www-form-urlencoded; charset=").append(respCharset).append("\r\n");
...
//            }
			...
			System.out.println("与[" + sendUri.getPath() + "]通信遇到异常,堆栈信息如下");
			e.printStackTrace();
		} finally {
	 ...
	 */
	private String formatToHexStringWithASCII(byte[] data){
	 ...
	 * @return 格式化后的十六进制字符串
	 */
	private String formatToHexStringWithASCII(byte[] data, int offset, int length){
	...
	private String getParamStr() throws UnsupportedEncodingException{
		...
		for (Map.Entry<String, String> entry : reqParams.entrySet()) {
			String keyName = URLEncoder.encode(entry.getKey(), charset.name()); 
		...
		if (reqData.length() > 0) {
			reqData = reqData.substring(0, reqData.length() - 1); //删除最后一个&符号
		}
	...
	public void setSendUri(URI sendUri) {
		this.sendUri = sendUri;
	}
	
	public void setSendUri(String sendUri) {
		this.sendUri = URI.create(sendUri);
	}
}
